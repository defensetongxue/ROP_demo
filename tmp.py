...
data_path='../autodl-tmp/HVDROPDB-RIDGE'
...
predict=[]
labels=[]
image_list=sorted(os.listdir(os.path.join(data_path,'images')))
    
with torch.no_grad():
    for image_name in image_list:
        # mask=Image.open(os.path.join(data_path,'masks',image_name)).resize((1600,1200),resample=Image.Resampling.BILINEAR).convert('L')
        # mask=np.array(mask)
        # mask[mask>0]=1
    

        img = Image.open(os.path.join(data_path,'enhanced_image',image_name))
        ...
        output_img=F.interpolate(output_img,(1200,1600), mode='nearest').squeeze()
        # output_img=output_img.squeeze()*mask
        ...
        #     else:
        visual_mask(
            os.path.join(data_path,'images',image_name),output_img.squeeze(),text_left=f"{max_val:.2f}",save_path=os.path.join('./experiments/HVD',image_name))
        # labels.append(tar)
        # predict.append(pred)
        # break
